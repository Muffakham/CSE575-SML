...
batch = torch.unsqueeze(imageTensor,0) # the function unsqueeze adds a dimension to a given tensor, over here
                                       ...
                                       # i.e it creates a tensor of images to form a batch
                                       # but we have only 1 image, so it creates a batch consisting of only 1 image
...
# evaluating the class of the image using the pretrained model alexnet

alex.eval() # putting the model into evaluation mode
output = alex(batch) # the data is fed to the mdoel and the output tensor is obtained

[values,index] = torch.max(output,1)  # the max function returns the value of the maximum tensor and its corresponding index
#percentage = torch.nn.functional.softmax(output, dim=1)[0] * 100    # basically creates the percentages of various classes
#print("percentage is ")
...
for image in image_list:
    ...
    imageTransformedVector.append(out_tensor) 

    diff_tensor = out_tensor.add(-output) # claculating the difference invector between given image and this image
    sqr_tensor = diff_tensor*diff_tensor # squaring the value to remove negative values
    x = torch.sum(sqr_tensor) # calculating the net sum of the output vector, ie the sum of all the values in a vector(tensor)
    netValueMatrix[inds] = (x.tolist()) #storing the difference values 
    inds += 1


#tt =sorted(netValueMatrix) 



tt=sorted(netValueMatrix.items(), key = lambda kv:(kv[1], kv[0])) # sorting the error matrix in ascending order
...
for f in tt:
    if(n==6):
        ...
        d = f[0]
        print(image_list[d]) # selects the image from the list
        v = Image.open(image_list[d])
        v.show() # displays the image
        [val,ind] = torch.max(imageTransformedVector[d],1) # determines the class of the image
        print(classLabels[ind.tolist()[0]]) # displays the class name of the image
        n+=1
